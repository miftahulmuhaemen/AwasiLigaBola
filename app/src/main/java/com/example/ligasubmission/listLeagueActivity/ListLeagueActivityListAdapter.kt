...
class ListLeagueActivityListAdapter(private var events: List<Event>, private val listener: (Event) -> Unit) :
        ...
        return object : Filter() {
            override fun performFiltering(charSequence: CharSequence): FilterResults {
                ...
                if (charString.isEmpty()) {
                    ...
                    for (row in events) {
                        if (row.strHomeTeam!!.toLowerCase().contains(charString.toLowerCase()) || row.strAwayTeam!!.contains(
                                charSequence
                            )
                        ) {
